...
def evaluate(epoch, epochs, model, test_loader, highest_acc):
    ...
    with torch.no_grad():  # turn off gradient computation to save memory and computation time
        ...
        for i, (_, text, audio, face, hrv, eye, y) in enumerate(test_loader):
            ...
            face = np.reshape(face, (face.shape[0], face.shape[1]*face.shape[2]))
            face = face.to(device)
        ...
        if current_acc == max(test_accuracy_record):
...
class GrowthMindsetModel(nn.Module):
    def __init__(self, output_dim=1):
        super().__init__()
        self.text_encoder = BiLSTMEncoder(768, 128, 256)
    ...
    def forward(self, text_seq, audio_seq, face_seq, hrv_feat, eye_feat, lengths=None):
        text_vec = self.text_encoder(text_seq, lengths)
        audio_vec = self.audio_encoder(audio_seq, lengths)
        face_vec = self.face_encoder(face_seq, lengths)
        hrv_vec = self.hrv_encoder(hrv_feat)
        eye_vec = self.eye_encoder(eye_feat)
...
if __name__ == '__main__':
    ...
    # ------------------- Predict function -------------------
    if isPredict:
        model = torch.load(predictModel).to(device)
    ...
    # Stratified split
    splitter = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=253)
    train_idx, test_idx = next(splitter.split(sid_list, label_list))
    ...
    train_labels = [dataset[i][6] for i in train_indices]
    test_labels = [dataset[i][6] for i in test_indices]
    print("Train Label Distribution:", Counter(train_labels))
    print("Test Label Distribution:", Counter(test_labels))
    
    # labels = [dataset[i][6] for i in range(len(dataset))]   # 假設 label 在索引 6

    # # 2. 使用 StratifiedShuffleSplit 直接把樣本切 8:2
    # splitter = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    # train_idx, test_idx = next(splitter.split(range(len(dataset)), labels))

    # # 3. 依照 index 建立 Subset 與 DataLoader
    ...
    # # 4. 檢查標籤分佈
    # train_labels = [dataset[i][6] for i in train_idx]
    # test_labels  = [dataset[i][6] for i in test_idx]
    # print("Train Label Distribution:", Counter(train_labels))
    # print("Test  Label Distribution:", Counter(test_labels))
    ...
    #                 n_hrv=23,
    #                 n_eye=7,
    #                 lstm_hiddens=[771, 193, 256, 23, 7], # 輸出層
    #                 text_lstm_layers=1,
    #                 audio_lstm_layers=1,
    #                 face_lstm_layers=1,
    #                 hrv_lstm_layers=1,
    ...
    # # 使用 MAML 訓練模型
    # best_accuracy = 0.0
    # save_dir = f"/root/MatureDepressionAssessment/ru_test/meta_learning/{timeStringOnly}"
    # os.makedirs(save_dir, exist_ok=True)  # 創建目錄（如果不存在）

    # # 保存混淆矩陣圖
    # save_path = os.path.join(save_dir, f"best_model.pth")
    # for epoch in range(epochs):
    #     # avg_loss, train_accuracy = maml_train(train_dataset, model, inner_lr, outer_optimizer, inner_steps=1)
    #     avg_loss, train_accuracy = maml_train_fixed_split(train_dataset, model, inner_lr, outer_optimizer)
    #     train_losses.append(avg_loss)
    #     train_accuracies.append(train_accuracy)
    #     print(f"Epoch {epoch + 1}/{epochs} - Loss: {avg_loss:.4f}, Accuracy: {train_accuracy:.2f}%")
    #     test_loss, test_accuracy = maml_evaluate(test_dataset, model, epoch)
    ...
    #         # print(f"New best model saved with accuracy: {best_accuracy:.2f}% at epoch {epoch + 1}")
    # # 繪製損失曲線
    # plt.figure(figsize=(12, 5))
    # plt.subplot(1, 2, 1)
    # plt.plot(range(1, epochs + 1), train_losses, marker='o', linestyle='-', color='blue', label="Train Loss")
    # plt.plot(range(1, epochs + 1), test_losses, marker='o', linestyle='-', color='red', label="Test Loss")
    # plt.xlabel("Epoch")
    # plt.ylabel("Loss")
    ...
    # # 繪製準確率曲線
    # plt.subplot(1, 2, 2)
    # plt.plot(range(1, epochs + 1), train_accuracies, marker='o', linestyle='-', color='blue', label="Train Accuracy")
    # plt.plot(range(1, epochs + 1), test_accuracies, marker='o', linestyle='-', color='red', label="Test Accuracy")
    # plt.xlabel("Epoch")
    # plt.ylabel("Accuracy (%)")
    ...
    # # 保存混淆矩陣圖
    # save_path = os.path.join(save_dir, f"maml_train_fixed_split.png")
    # # save_path = f"/root/MatureDepressionAssessment/ru_test/meta_learning/{timeStringOnly}/maml_train.png"  # 修改為您想要的路徑和文件名
    # plt.tight_layout()
    ...
    #     train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    #     test_loader = DataLoader(test_dataset, batch_size=1)  # Batch size is 1 for testing
    # else:
    ...
    #     print('train data num:', len(train_indices), '\ntest data num: ', len(test_indices))

    #     # 幫 dataset 加上權重，平衡 dataset
    #     # class_weights = 1.0 / torch.tensor(
    #     #     [dataset.y_label_qun[label] for label in dataset.Y_label]
    #     # )
    #     # sample_weights = [class_weights[label] for label in dataset.Y_label]

    #     # create samplers using weighted random sampler
    #     # train_sampler = WeightedRandomSampler(sample_weights, num_samples=len(dataset) - split, replacement=True)
    #     # test_sampler = WeightedRandomSampler(sample_weights, num_samples=split, replacement=True)

    #     # train_indices = train_indices[:len(train_indices) - len(train_indices)%256]
    #     # test_indices = test_indices[:len(test_indices) - len(test_indices)%256]
    ...
    f.write(f"LR: {LR}\n")
    f.write(f"test_split( useShuffleSplit : { useShuffleSplit } ): {test_split}\n")
    f.write(f"shuffle_dataset: {shuffle_dataset}\n")
    f.write(f"dataset_size: {dataset_size}\n")
    f.write(f'samples : {dataset.sample_quntity}\n')  
    f.write(f"dataset for each category: {dataset.y_label_qun}\n")
    print(f'train data num: {len(train_loader)} \ntest data num: {len(test_loader)}', file=f)
    f.write("----------\n\n")
    f.close()
    ...
    for epoch in range(epochs):
        ...
        for i, (_, text, audio, face, hrv, eye, y) in enumerate(train_loader):
            ...
            #                 text.to(dtype=torch.float), 
            #                 audio.to(dtype=torch.float), 
            #                 face.to(dtype=torch.float),
            #                 hrv.to(dtype=torch.float), 
            #                 eye_movement.to(dtype=torch.float),
            #                 )
            loss = loss_fn(outputs, y.to(device))     # 计算两者的误差
            
            train_loss += loss.item()
            train_acc += accuracy(outputs.cpu(), y)
            count += 1
            
            _, predicted_labels = torch.max(outputs.cpu(), 1)
            predict.extend(predicted_labels.tolist())
        ...
        print(cf_matrix) #打印出来看看
        # 打印每个 epoch 的损失值和准确率
        print(f"Epoch [{epoch+1}/{epochs}], Loss: {train_loss/(i+1):.4f}, \
            Accuracy: {train_acc/(i+1):.4f}")
        
        loss_record.append(round(train_loss/(i+1), 4))
        accuracy_record.append(round(train_acc/(i+1), 2))
        ...
        f.write("----------\n")
        print(f"Epoch [{epoch+1}/{epochs}], Loss: {train_loss/(i+1):.4f}, \
            Accuracy: {train_acc/(i+1):.4f}", file=f)
        print(f'{cf_matrix}', file=f)
        f.write("----------\n\n")
        f.close()
        ...
        if (epoch+1) % 1 == 0:
            highest_acc = evaluate(epoch+1, epochs, model, test_loader, highest_acc)
        
    
    plt.clf()
    # 訓練時期數量
    epochs = range(1, len(accuracy_record) + 1)
    plt.plot(epochs, accuracy_record, 'b-o')
    plt.title('Accuracy per Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.xticks(epochs)
    plt.yticks(accuracy_record)
    plt.grid(True)
    plt.savefig(f'{experimentPath}/{timeString}/Accuracy per Epoch', dpi=300, bbox_inches='tight')
    plt.close()

    plt.clf()
    # 訓練時期數量
    epochs = range(1, len(loss_record) + 1)
    plt.plot(epochs, loss_record, 'b-o')
    plt.title('Loss per Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Loss Value')
    plt.xticks(epochs)
    plt.yticks(loss_record)
    plt.grid(True)
    plt.savefig(f'{experimentPath}/{timeString}/Loss per Epoch', dpi=300, bbox_inches='tight')
    plt.close()

    plt.clf()
    # 訓練時期數量
    epochs = range(1, len(accuracy_record) + 1)
    plt.plot(epochs, test_accuracy_record, 'b-o')
    plt.title('Accuracy per Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.xticks(epochs)
    plt.yticks(test_accuracy_record)
    plt.grid(True)
    plt.savefig(f'{experimentPath}/{timeString}/Test accuracy per Epoch', dpi=300, bbox_inches='tight')
    plt.close()

    plt.clf()
    # 訓練時期數量
    epochs =range(1, len(loss_record)  + 1) 
    plt.plot(epochs, test_loss_record, 'b-o')
    plt.title('Loss per Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Loss Value')
    plt.xticks(epochs)
    plt.yticks(test_loss_record)
    plt.grid(True)
    plt.savefig(f'{experimentPath}/{timeString}/Test loss per Epoch', dpi=300, bbox_inches='tight')
    plt.close()
    
    f = open(f'{experimentPath}/{timeString}/{timeString}_train.txt', 'a')
    f.write("----------\n")
    print(f"{model_name}", file=f)
    for i in test_accuracy_record:
        print(f"{i}", file=f)
    print(f'{cf_matrix}', file=f)
    f.write("----------\n\n")
    f.close()
